...
function TableHeaderCell({
  ...
  return (
    <TableHead
      className={
        sortKey.includes("Eps") ||
        sortKey === "marketCap" ||
        sortKey === "numberOfEstimates"
          ? "text-right"
          : ""
      }
    >
...
const sortByMarketCap = (
  data: Earning[],
  direction: SortDirection = SORT_DIRECTIONS.DESC
) => {
  return [...data].sort((a, b) => {
    const marketCapA = parseFloat(
      (a.marketCap ?? "0").replace(/[^0-9.-]+/g, "")
    );
    const marketCapB = parseFloat(
      (b.marketCap ?? "0").replace(/[^0-9.-]+/g, "")
    );
    return direction === SORT_DIRECTIONS.ASC
...
export default function EarningsTable({
  ...
  const formatDate = (date: Date) => {
    ...
    return `${localDate.getDate()} ${localDate.toLocaleString("default", {
      month: "short",
    })}`;
  };
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHeaderCell
                label="Estimated EPS"
                sortKey="estimatedEps"
                onSort={handleSort}
              />
              <TableHeaderCell
                label="Fiscal Quarter Ending"
                sortKey="fiscalQuarterEnding"
                onSort={handleSort}
              />
              <TableHeaderCell
                label="Market Cap"
          ...
          <TableBody>
            {filteredAndSortedData.map((earning) => (
              ...
              >
                ...
                <TableCell className="text-center">
                  {earning.fiscalQuarterEnding}
                </TableCell>
                <TableCell className="text-center">
                  {earning.marketCap}
                </TableCell>
                <TableCell className="text-center">
                  {earning.numberOfEstimates}
                </TableCell>
                <TableCell>
                  <Button
                    variant="ghost"
